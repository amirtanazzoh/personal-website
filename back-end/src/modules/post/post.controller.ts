...
export class PostController
{
    ...
    @Post()
    async create ( @Body() body: CreatePostDto ) { return this.service.create( body ); }

    @Get( ':id' )
    async getById ( @Param( 'id', ParseIntPipe ) id: number ) { return this.service.getById( id ); }

    @Put( ':id' )
    async update ( @Body() body: UpdatePostDto, @Param( 'id', ParseIntPipe ) id: number ) { return this.service.update( id, body ); }
